...
function init() {
    ...
    s += '<canvas id="fId" style="display:block; margin:auto; z-index:1; text-align: center;"></canvas>'
    ...
    s += '</div>'
    docInsert(s)
    my.can = new Can('canvasId', 100, 100, 1)
    my.jig = document.getElementById('jig')
    my.fileName = 'eiffel.jpg'
...
function getPopHTML() {
    ...
    let opts = []
    for (let i = 2; i <= 12; i++) { opts.push({ name: i, descr: i }) }
    s += wrap({ id: 'rowN', tag: 'sel', lbl: 'Rows:', txt: '2', opts: opts })
    s += ' &nbsp;  '
    s += wrap({ id: 'colN', tag: 'sel', lbl: 'Cols:', txt: '2', opts: opts })
...
function editYes() {
    ...
    my.rowN = Math.max(2, Math.min(my.rowN >> 0, 12))
    my.colN = document.getElementById('colN').value
    my.colN = Math.max(2, Math.min(my.colN >> 0, 12))
    if (my.imgQ) { loadImg(0.5, true) } else { loadImg(0, true) }
}
function doAnim() {
    /////////////////////////////Animation/////////////////////////////
    let pc = my.p[my.animN]
    if (my.animTp % 2 == 0) {
    ...
    my.animN++
    if (my.animN < my.rowN * my.colN) { requestAnimationFrame(doAnim) } else {
        my.animN = 0
        my.animTp++
...
function loadImg(alpha, newQ) {
    ...
    image.onload = function () {
        ...
        // console.log('My ratio'+ my.can.ratio)
...
function getRandPts() {
    ...
    let xCurr = -rect.left
    let yCurr = 0
    for (let i = 0; i < my.colN * my.rowN; i++) {
        pts.push({ x: xCurr + Math.random() * my.pWd, y: yCurr + Math.random() * my.pHt, })
        xCurr += my.pWd * 1.5
        if (xCurr >= document.body.clientWidth - rect.left - my.pWd * 2) {
            xCurr = -rect.left
            yCurr += my.pHt * 1.5
        }
    }
...
function go(img) {
    ...
    let p = []
    let count = 0
    for (let l = 0; l < my.rowN; l++) {
        for (let k = 0; k < my.colN; k++) {
            p.push(new PuzzlePiece({ rotation: 0, imgwidth: img.width, imgheight: img.height, width: my.pWd, height: my.pHt, image: img, imagepoint: new Point(k * my.pWd, l * my.pHt), top: { first: new Point(k * my.pWd, l * my.pHt), last: new Point(k * my.pWd + my.pWd, l * my.pHt), }, right: { first: new Point(k * my.pWd + my.pWd, l * my.pHt), last: new Point(k * my.pWd + my.pWd, l * my.pHt + my.pHt), }, bot: { first: new Point(k * my.pWd + my.pWd, l * my.pHt + my.pHt), last: new Point(k * my.pWd, l * my.pHt + my.pHt), }, left: { first: new Point(k * my.pWd, l * my.pHt + my.pHt), last: new Point(k * my.pWd, l * my.pHt), }, }))
            p[p.length - 1].canvas.style.setProperty('left', pts[ptNo].x + 'px')
            p[p.length - 1].canvas.style.setProperty('top', pts[ptNo].y + 'px')
            ptNo++
            if (k !== 0) {
                let temp = p[p.length - 2].right.points
            ...
            if (l !== 0) {
                let temp2 = p[count - my.colN].bot.points
...
let PuzzlePiece = (function () {
    function PuzzlePiece(obj) {
        ...
        if (obj.left.first.x > 0) { this.left.points = this.makeSide(this.left) } else {
            this.left.points = []
            this.left.points.push(this.left.first)
            this.left.points.push(this.left.last)
        }
        if (obj.top.first.y > 0) { this.top.points = this.makeSide(this.top) } else {
            this.top.points = []
        ...
        } else { this.right.points = this.makeSide(this.right) }
        ...
        } else { this.bot.points = this.makeSide(this.bot) }
...
PuzzlePiece.prototype = {
    rotate: function (around) {
        this.connected.forEach(function (element) {
            let temptop = parseFloat(around.canvas.style.top) - (parseFloat(around.canvas.style.left) - parseFloat(element.canvas.style.left))
        ...
        for (let i = 2; i < args.length - 1; i++) {
            let before = new Point(args[(i + (args.length - 1)) % args.length].x, args[(i + (args.length - 1)) % args.length].y)
...
function contains(a, obj) {
    for (let i = 0; i < a.length; i++) { if (a[i] === obj) { return true } }
    return false
}
Array.prototype.myUnique = function () {
    let r = []
    o: for (let i = 0; i < this.length; i++) {
        for (let x = 0; x < r.length; x++) { if (r[x] === this[i]) { continue o } }
        r[r.length] = this[i]
    }
    return r
}
function checkForMatches(e) {
    let top = parseFloat(e.canvas.style.top)
    ...
    if (typeof e.piecetop !== 'undefined') {
        let toptop = parseFloat(e.piecetop.canvas.style.top)
    ...
    if (typeof e.pieceright !== 'undefined') {
        let righttop = parseFloat(e.pieceright.canvas.style.top)
    ...
    if (typeof e.pieceleft !== 'undefined') {
        let lefttop = parseFloat(e.pieceleft.canvas.style.top)
    ...
    if (typeof e.piecebot !== 'undefined') {
        let bottop = parseFloat(e.piecebot.canvas.style.top)
...
function checkDone(conn) {
    conn = my.p[0].connected
    if (conn.length == my.colN * my.rowN) { 
        let num =$('#rowN').val();
        ...
        if(num==2){
         ...
         var level = +num + 3;
         alert('this is the number'+aaaaa+"--"+level);
        if(aaaaa < level){
            // alert('Game completed succefulley')
            aaaaa++;
            optPop()
            alert('Game is completed you need to');
            editYes()
            ...
            $('#colN').val(aaaaa);
            l++;  
            alert('Congeratulation level '+ l +' is completed successfully');
            jiggle()
...
function wrap({ id = '', cls = '', pos = 'rel', style = '', txt = '', tag = 'div', lbl = '', fn = '', opts = [] }, ...mores) {
    ...
    s += {
        ...
        }, can: () => '<canvas', div: () => '<div', edit: () => '<textarea onkeyup="' + fn + '" onchange="' + fn + '"', inp: () => {
    ...
    s += {
        btn: () => '>' + txt + '</button>', can: () => '></canvas>', div: () => ' >' + txt + '</div>', edit: () => ' >' + txt + '</textarea>', inp: () => '>' + (lbl.length > 0 ? '</label>' : ''), out: () => ' >' + txt + '</span>' + (lbl.length > 0 ? '</label>' : ''), rad: () => {
            let s = ''
            s += '>\n'
